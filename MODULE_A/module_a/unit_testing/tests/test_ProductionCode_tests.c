...
TEST(ProductionCode, FindFunction_WhichIsBroken_ShouldReturnTheIndexForItemsInList_WhichWillFailBecauseOurFunctionUnderTestIsBroken)
{
  ...
  TEST_ASSERT_EQUAL(0, FindFunction_WhichIsBroken(8888));
}
...
TEST(ProductionCode, FnRetsLocVar_ShouldReturnCurrentCounter_FailsBCItsTestIsFlawed)
{
    //Sometimes you get the test wrong.  When that happens, you get a failure too... and a quick look should tell
    // you what actually happened...which in this case was a failure to setup the initial condition.
    TEST_ASSERT_EQUAL_INT(FunctionWhichReturnsLocalVariable()-1, FunctionWhichReturnsLocalVariable());
}
